...
function solidToHex(paints) {
  const solid = paints?.find((p) => p.type === "SOLID" && "color" in p);
  return solid ? rgbToHex(solid.color) : void 0;
}
async function exportStyles() {
  ...
  for (const s of figma.getLocalTextStyles()) {
    text[s.name] = {
      ...
      size: s.fontSize,
      lineHeight: s.lineHeight?.value ?? null,
      letterSpacing: s.letterSpacing?.value ?? null,
...
function shapeSchema(styles, vars) {
  const color = {};
  ...
  const tokens = {
    ...
    typography: {
      fontFamily: { base: styles.text && Object.values(styles.text)[0] && Object.values(styles.text)[0].fontFamily || "Inter" },
      size: { xs: 12, sm: 14, md: 16, lg: 20, xl: 24, "2xl": 32 },
      weight: { regular: 400, medium: 500, semibold: 600 }
    },
    charts: { palette: ["#0A6EEB", "#2ED3B7", "#F59E0B", "#EF4444", "#8B5CF6"] },
    meta: { source: "figma-plugin", exportedAt: (/* @__PURE__ */ new Date()).toISOString() }
  };
  return tokens;
}
